...
- structuring sequences of data operations left-to-right (as opposed to from the inside and outside),
...
flights %>% slice_max(air_time, n = 3)
...
```{r}
df %>% 
  full_join(tibble(a = 3, b = "z", c = 3.5))
```

Inserting rows
```{r}
df %>% 
  rows_insert(tibble(a = 4, b = "z", c = 3.5))
# or explictly
df %>% 
  rows_insert(tibble(a = 4, b = "z", c = 3.5), by = "a")
```

```{r, error = TRUE}
# error because `a = 3` exists
df %>% 
  rows_insert(tibble(a = 3, b = "z", c = 3.5))
```

Use `rows_update` to update existing rows
```{r}
df %>% 
  rows_update(tibble(a = 3, b = "z", c = 3.5))
```

`rows_patch` works like rows_update() but only overwrites NA values.
```{r}
df %>% 
  rows_patch(tibble(a = 3, b = "z", c = 3.5))
```
```{r, error = TRUE}
# error when key doesn't exist
df %>% 
  rows_patch(tibble(a = 4, b = "z", c = 3.5))
```

`rows_upsert` inserts or updates depending on whether or not the key value already exists
```{r}
df %>% 
  rows_upsert(tibble(a = 3:4, b = c("x", "y"), c = c(3.5, 4.5)))
```
